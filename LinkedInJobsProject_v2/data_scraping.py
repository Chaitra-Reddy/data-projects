...
# modify the job title to append into the URL. "%2B" represents a space in hexadecimal
job_title_words = CONSTANTS.JOB_TITLE.split(" ")
job_title = ""
for i,each in enumerate(job_title_words):
    if i != len(job_title_words)-1:
        job_title += each + '%2B'
    else:
        job_title += each
...
for i in range(0, no_of_loops):
    resp = requests.get(CONSTANTS.JOBS_LIST_URL.format(job_title, 25*(i+1)))
    ...
    for j in range(0, len(all_jobs_current_page)):
        ...
        each_job = dict()
        resp = requests.get(CONSTANTS.JOB_DETAILS_URL.format(job_id))
        soup = BeautifulSoup(resp.text, "html.parser")
